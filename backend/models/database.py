...
def init_database():
    ...
    # Migration guard: add Part 2 email-trigger columns to shipments tables
    # created before this feature existed. SQLite has no "ADD COLUMN IF NOT
    # EXISTS", so probe the column list instead.
    existing_cols = {row["name"] for row in conn.execute("PRAGMA table_info(shipments)")}
    migrations = {
        "source": "ALTER TABLE shipments ADD COLUMN source TEXT DEFAULT 'upload'",
        "sender_email": "ALTER TABLE shipments ADD COLUMN sender_email TEXT",
        "subject": "ALTER TABLE shipments ADD COLUMN subject TEXT",
        "received_at": "ALTER TABLE shipments ADD COLUMN received_at TIMESTAMP",
    }
    ...
    try:
        ...
        conn.execute("""
            ...
            (id, customer_id, customer_name, status, decision, decision_reasoning,
             ...
             source, sender_email, subject, received_at,
