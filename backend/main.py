...
from backend.services.inbox_watcher import (
    get_inbox_items,
...
# --- Inbox Endpoints (Part 2 — trigger) ---

@app.get("/api/inbox")
async def list_inbox():
    """List everything the simulated SU inbox has seen: new, processing, or
    processed emails. Each item links to its shipment_id once the pipeline
    has run, so the UI can jump straight to the verification result."""
    return get_inbox_items()


@app.post("/api/inbox/simulate")
async def simulate_email(seed: Optional[str] = None):
    """Drop one canned SU email into the watched inbox folder so a demo can
    show a "new email just arrived" moment without waiting on real
    filesystem timing. The watcher picks it up and triggers the pipeline
    exactly as it would for a real dropped file."""
    try:
        result = simulate_incoming_email(seed)
