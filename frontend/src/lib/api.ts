...
export async function simulateInboxEmail(seed?: string): Promise<{ status: string; id: string; seed: string }> {
  const qs = seed ? `?seed=${encodeURIComponent(seed)}` : '';
  const res = await fetch(`${API}/inbox/simulate${qs}`, { method: 'POST' });
  return res.json();
}
